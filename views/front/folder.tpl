...
<h2 class="accordion_header card-header">{if $product->name}{l s=$product->name mod='clariprint'}{else}{l s='Folder' mod='clariprint'}{/if}{if $remove_product}<span class="remove_product ui-button-icon ui-icon ui-icon-closethick" style="float: right">{/if}</h2>{/if}
