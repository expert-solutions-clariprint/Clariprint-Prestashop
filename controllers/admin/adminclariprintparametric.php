<?php
require_once _PS_MODULE_DIR_ . '/clariprint/classes/parametricasset.php';
...
class AdminClariprintParametricController extends ModuleAdminController
{

	public function __construct()
	{
		$this->bootstrap = true;
		$this->table = QypAsset::$definition['table']; //Table de l'objet
		$this->identifier = QypAsset::$definition['primary']; //Clé primaire de l'objet
		$this->className = QypAsset::class; //Classe de l'objet

		if (Tools::getValue('id_qyp_asset'))
		{
	...
	public function renderForm()
	{
		...
		if (Tools::getValue('action') == "SaveAsset")
		{
			...
			if ($obj->id && $file )
			{
				$dir = _PS_ROOT_DIR_.'/img/qyp/'.$obj->id;
				@mkdir($dir,0777,true);
		...
		$this->context->smarty->assign(['assets' => Db::getInstance()->executeS(sprintf("SELECT id_qyp_asset,name,uuid,kind,
					if(kind = 'FILE',json,null) as json
						FROM %s%s order by name",_DB_PREFIX_,QypAsset::$definition['table']))]);
		...
		$this->context->smarty->addTemplateDir(_PS_MODULE_DIR_.'/quoteyourpack/views/front');
	...
	private function buildRequest($id,$action='GetSVG', $params = null)
	{
		...
		while(count($toload)) {
			...
			if (is_string($r))
			{
				...
				if ($uuid)
					$res = Db::getInstance()->executeS(sprintf("SELECT kind,json,name FROM %sqyp_asset WHERE uuid = '%s' ",_DB_PREFIX_,$uuid));
				else {
					$res = Db::getInstance()->executeS(sprintf("SELECT kind,json,name FROM %sqyp_asset WHERE name = '%s' ",_DB_PREFIX_,$name));
				}
				if (count($res) > 0)
				{
					$row = $res[0];
		...
		*/
		...
		curl_setopt_array($curl, array(
		  CURLOPT_URL => 'http://109.190.56.28:8024/optimproject/json.wcl',
	...
	function displayAjaxSamplerJS()
	{
		...
		curl_setopt_array($curl, array(
		  CURLOPT_URL => 'http://109.190.56.28:8024/js/xl_3D_sampler.js',
		  CURLOPT_RETURNTRANSFER => true,
		  CURLOPT_ENCODING => '',
		  CURLOPT_MAXREDIRS => 10,
		  CURLOPT_TIMEOUT => 0,
